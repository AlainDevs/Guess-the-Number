@IBAction func guessButtonPressed(_ sender: UIButton) {
    ...
    if guess == secretNumber {
        ...
        if guess < secretNumber {
            lives -= 1
            message = "Sorry, that's not the secret number. Your guess is too low. You have \(lives) lives left."
        } else {
            lives -= 1
            message = "Sorry, that's not the secret number. Your guess is too high. You have \(lives) lives left."
        }
        if score > 0 {
            score -= 1
        }
